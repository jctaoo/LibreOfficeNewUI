                                            ...
                                            const OUString&, const Color& rBackgroundColor)
{
    ...
    if (nType == ControlType::TabBody) {
        ...
        const OUString cssString =
            "* { "
            "   border-radius: 8px; "
            "   padding: 15px; "
            "   box-shadow: 0px 8px 8px -12px rgba(92,86,86,0.79), 0px -1px 4px -3px rgba(92,86,86,0.79); "
            "}";
        const OString o_s = OUStringToOString(cssString, RTL_TEXTENCODING_UTF8);
        css_provider_load_from_data(custom_provider, o_s.getStr(), o_s.getLength());
        gtk_style_context_add_provider(context, GTK_STYLE_PROVIDER(custom_provider), GTK_STYLE_PROVIDER_PRIORITY_APPLICATION);
    }
...
bool GtkSalGraphics::updateSettings(AllSettings& rSettings)
{
    ...
    // JC TODO: 看看 aBackColor 如何获取暗黑模式和明亮模式的背景颜色
    aStyleSet.BatchSetBackgrounds( Color(236, 241, 244) );
